...
def render_radiance_volume(r_ts, ds, z_is, chunk_size, F, t_is):
    ...
    for chunk_start in range(0, r_ts_flat.shape[0], chunk_size):
        ...
        # n = np.shape(preds["sigma_is"])[0]
        # for i in range(n):
        #     if preds["sigma_is"][i] < 0.2:
        #         preds["sigma_is"][i] = 0

        # sgi = torch.tensor([]).to(device="cuda:0")
        # # sgi.to(device="cuda:0")
        # for i in range(n):
        #     if preds["sigma_is"][i] < 0.2:
        #         torch.cat((sgi, torch.tensor([0]).to(device="cuda:0")), 1)
        #     else:
        #         torch.cat((sgi, preds["sigma_is"][i]), 1)
...
def main():
    ...
    for tr_set in range(5):
        ...
        # train_dataset = load_data2(n_objs)
        # camera_distance = train_dataset[0].camera_distance
        # scale = train_dataset[0].scale
        # img_size = train_dataset[0][0][2].shape[0]
        ...
        for i in range(num_iters):
            ...
            if i % display_every == 0:
                ...
                plt.title("PSNR")
                # plt.show()
                plt.savefig(f"results/{tr_set}_{i}.png")
                F_c.train()
                F_f.train()
